                ...
                <div class="comments" v-for="comment of commentList">
                    ...
                    <div class="comments__ask">
                        <div class="comments__ask__header">
                            ...
                            <span class="name">{{comment.name}}</span>&ensp;·&ensp;<span>{{comment.time | moment "from" "now"}}</span><span class="hidden-xs">&ensp;·&ensp;<span class="reply" @click="replyBtn(comment._id)">回复</span>{{selectId}}--{{toggle}}</span>
                        </div>
...
<script>
    ...
    import {getArticleById,getArticleComments,clearArticle} from '../vuex/actions'
    ...
    module.exports = {
        ...
        ready: function () {
             ...
             * 初始化
             * */
            this.clearArticle(); // 初始状态,监听状态变化
            this.$watch('article', function (val) {
...
//            localStorage.setItem("a","hello")
            let commentInfo = localStorage.commentInfo;
            console.log(!!commentInfo && !!commentInfo.name && !!commentInfo.email)
            console.log(commentInfo)
            console.log(commentInfo = JSON.parse(commentInfo))
            console.log(commentInfo.email)
            if(!!commentInfo && !!commentInfo.name && !!commentInfo.email){
                this.hasNickName = true;
                this.name = commentInfo.name;
                this.email = commentInfo.email;
            }else{
                this.hasNickName = false;
            }
            console.log(commentInfo)
            console.log(this.hasNickName)
        ...
        events:{
            'replyThisComment':function (data) {
                if(this.hasNickName){
                    ...
                    // send
                    console.log("hasNickName:"+params)
                }else{
                    if(!!data.name && !!data.email){
                        localStorage.commentInfo =JSON.stringify({name:data.name, email:data.email});
                        this.hasNickName=true;
                        let params = {
                            article_id: data.article_id,
                            pre_id:  data.pre_id,
